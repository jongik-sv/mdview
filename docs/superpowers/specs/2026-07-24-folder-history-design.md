...
## 후속: 외부 마커의 이동 대상 (v0.1.17)

마커가 늘 파일의 직속 폴더를 열어서, 하위 폴더의 문서 하나를 열면 프로젝트
루트가 그 하위로 좁혀졌다 — 트리가 쓸모없어진다.

`bestFolderFor(path)`: 히스토리의 `kind === 'folder'` 항목 중 그 파일을 품는
(`isUnderDir`) 첫 항목을 연다. 히스토리는 최신순이므로 후보가 여럿이면 가장
최근에 쓰던 프로젝트가 선택된다. 품는 항목이 없으면 종전대로 `parentDir`.

마커 툴팁에 열릴 폴더 이름을 넣었다 — 직속 폴더가 아닐 수 있어서 밝히지 않으면
어디로 갈지 예측할 수 없다. 탭 우클릭의 "이 파일의 폴더 열기"는 그대로
`parentDir`을 연다 (직속 폴더를 확실히 여는 탈출구).
